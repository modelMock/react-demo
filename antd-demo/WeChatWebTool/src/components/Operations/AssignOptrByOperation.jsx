...
import { queryOperationForOptrIdIsNul, updateAssignOptr, updateOperation }  from '../../services/operations';
...
class SelectConfirmForm extends Component {
  ...
  handleSelectiveSubmit() {
    this.props.form.validateFields( (errors, values) => {
      ...
      }.bind(this), "确定要给表格中选中运营号数据分配当前客服吗?", "可选择单行或多行进行分配");
  ...
  handleAllSubmit() {
    this.props.form.validateFields( (errors, values) => {
      ...
      Confirm(function(){
        ...
        console.log('handleAllSubmit => ', values);
        this.submitAssignOptr(values);

      }.bind(this), `确定要给表格中所有运营号数据(共${this.props.totalCount}个)分配当前客服吗?`, "对整个表格数据进行分配");
    });
  }

  //private:提交数据后重置界面组件、表单数据
  submitAssignOptr(values) {
    updateAssignOptr(values).then( ({jsonResult}) => {;
        Success('分配客服成功');
        this.setState({
  ...
  componentWillReceiveProps(nextProps) {
    ...
    console.log('componentWillReceiveProps => ', isDisabledSingleBtn, isDisabledMulitBtn);
  ...
  render() {
    ...
    const { selectedRowKeys, friendsTotal, totalCount, searchObj } = this.props;
        ...
        <Col sm={9}>
          <OptrSelectField
            getFieldDecorator={getFieldDecorator('service_optr_id', {
                rules:[{type:'object', required:true, message:'请选择客服'}]
              })}/>
        </Col>
        <Col sm={9}>
          <FormItem labelCol={{ span:8 }} wrapperCol={{ span: 16　}} label="选择微信号">
            <p className="ant-form-text" id="userName" name="userName" style={{ color:'red' }}>
              已选择{selectedRowKeys.length}个运营号，共{friendsTotal}好友
            </p>
          </FormItem>
        </Col>
        <Col sm={3}>
            <Button type="primary" icon="check" size="large"
                disabled={ isDisabledSingleBtn } onClick={this.handleSelectiveSubmit}>
              选择提交
            </Button>
        </Col>
        <Col sm={3}>
...
//按运营号分配
export default class AssignOptrByOperation extends Component {
  ...
  render() {
    console.log('AssignOptrByOperation => render => ', this.state);
    ...
    return (
      <div>
        ...
        <CommonTable ref="commontable"
          ...
          rowKey = "operation_sn"
          scroll={{x: 880}}
          fetchTableDataMethod = { queryOperationForOptrIdIsNul }
