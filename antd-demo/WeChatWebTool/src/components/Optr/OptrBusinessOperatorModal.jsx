...
import {queryBusinessOperator,batchUpdateSysOptrBusinessOperator} from '../../services/optr';
const FormItem = Form.Item;
const Option = Select.Option;
/**
 * 客服设置运营方
 */
 class OptrBusinessOperatorModal extends React.Component{
   constructor(props){
     super(props);
     this.state={
       RowsDetails:props.businessOperatorDetails,
       visible: props.businessOperatorVisible,
       record: [],
       filterValue:[]
     };
   ...
   handleOk(){
     this.props.form.validateFields((errors, values) => {
       if(!!errors) return;
       Confirm(function(){
       batchUpdateSysOptrBusinessOperator({business_operator:values.business_operator,optr_ids:this.props.clusterSns}).then(({jsonResult}) => {
         Success("设置成功");
         this.handleCancel();
       });
     }.bind(this), "确定设置运营方吗?");
   ...
   render(){
     ...
     console.log("RowsDetails=>",RowsDetails);
     return (
       <Modal visible={visible} maskClosable={false} title={`设置运营方`}
         ...
         <Form horizontal>
           <FormItem label="群ID" {...formItemLayout} style={{marginTop: -12}}>
             <p>
               {
                RowsDetails.map((item, idx)=> <span style={{with:'60px'}}>{item.optr_id},</span>)
               }
            </p>
           </FormItem>
 ...
 export default Form.create()(OptrBusinessOperatorModal);
